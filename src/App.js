  ...
  const show = () => {
    ...
    for (let i = 0; i < rounds.length; i++) {
      for (let j = 0; j < rounds[i].matches.length; j++) {
        if (rounds[i].matches[j].score.ft[0] > rounds[i].matches[j].score.ft[1]) {
          const indClubWon = newClubs.findIndex((club) => club.name === rounds[i].matches[j].team1);
          newClubs[indClubWon] = {
          ...
          const indClubLost = newClubs.findIndex((club) => club.name === rounds[i].matches[j].team2);
        ...
        } else if (rounds[i].matches[j].score.ft[0] < rounds[i].matches[j].score.ft[1]) {
          const indClubWon = newClubs.findIndex((club) => club.name === rounds[i].matches[j].team2);
          newClubs[indClubWon] = {
            code: newClubs[indClubWon].code,
            country: newClubs[indClubWon].country,
            drawn: newClubs[indClubWon].drawn,
          ...
          const indClubLost = newClubs.findIndex((club) => club.name === rounds[i].matches[j].team1);
          newClubs[indClubLost] = {
            code: newClubs[indClubLost].code,
            country: newClubs[indClubLost].country,
            drawn: newClubs[indClubLost].drawn,
          ...
          const indClubTeam1 = newClubs.findIndex((club) => club.name === rounds[i].matches[j].team1);
          ...
          const indClubTeam2 = newClubs.findIndex((club) => club.name === rounds[i].matches[j].team2);
  ...
  return (
    <div className="App">
      <div className="container">
        ...
        <button className="" onClick={show} disabled={isShow}>
          Show
        </button>
